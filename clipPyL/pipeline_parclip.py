...
from clipPyL.chrom_db_io import ParClipSQLite
...
def build_db(in_sam_fp, in_cleav_fp, out_dir, clipped_read_filetype='fastq'):
    ...
    if in_cleav_fp == in_sam_fp:
        ...
        cleav_db = ParClipSQLite(cleav_db_fp)
    ...
    start_time = time.time()
    sam_db = ParClipSQLite(sam_db_fp)
    ...
    parclip_pyl_SQLite = ParClipSQLite(pyl_db_fp)
    bw_stat_dict = parclip_pyl_SQLite.build_basewise_signature( 
                                          sam_db_fp = sam_db_fp,
                                          cleav_db_fp = cleav_db_fp, 
                                          uniq_only = True )
    
    parclip_pyl_SQLite.conn.close()
    
    elapsed_time = time.time() - start_time
    print('positional par-clip signature was written and indexed to:')
    print(pyl_db_fp)
    print('The amount of time that elapsed during the process was:')
    print('{0:.2f}'.format(round(elapsed_time,2)) + ' seconds')
    print('#######################################')
    ...
    with open(stat_fp, 'w') as stat_fh:
        stat_fh.write('par-clip PyL was built with the following command line parameters:\n')
        ...
        stat_fh.write('Number of nucleotides with T to C: ' + str(bw_stat_dict['n_of_t_to_c_sites']) + '\n')
        ...
        #note: impacted by uniq_only
        stat_fh.write('Number of mapped T to C reads: ' + str(bw_stat_dict['n_of_mapped_t_to_c_reads']) + '\n')
...
def main(argv=None):
    ...
    try:
        try:
            d = '''Calculate the base-wise PAR-CLIP signature and buld the '.PyL.sqlite' database'''
            parser = argparse.ArgumentParser(description = d)
            
            parser.add_argument('-a', '--alignments', 
                                metavar='file path to alignments', type=str,
                                required=True,
                                help='''Provide the filepath to the read alignments from your PAR-CLIP
