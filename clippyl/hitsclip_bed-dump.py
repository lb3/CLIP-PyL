...
def hitsclip_bed-dump(  bam_fp_l,
    ...
    for bam_fp, readid_db_fp in zip(bam_fp_l, readid_db_fp_l):
        ...
        pysam_bam_file_conn = pysam.Samfile(bam_fp, "rb")
        ...
        with open(os.path.splitext(bam_fp)+'top_std_cov.bg','w') as bg_fh_top_strand_cov, \
             open(os.path.splitext(bam_fp)+'bot_std_cov.bg','w') as bg_fh_bot_strand_cov, \
             open(os.path.splitext(bam_fp)+'top_std_clv.bg','w') as bg_fh_top_strand_clv, \
             open(os.path.splitext(bam_fp)+'bot_std_clv.bg','w') as bg_fh_bot_strand_clv, \
             open(os.path.splitext(bam_fp)+'top_std_1D.bg','w') as bg_fh_top_strand_1D, \
             open(os.path.splitext(bam_fp)+'bot_std_1D.bg','w') as bg_fh_bot_strand_1D:
        
        hitsclip_vectors_2_bg( pysam_bam_file_conn,
                               sample_name,
